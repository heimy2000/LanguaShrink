...
# Load the Excel file
data_frame = pd.read_excel('NQ-open.dev1.xlsx', engine='openpyxl')

# Calculate the F1 score for each row
data_frame['f1_score'] = data_frame.apply(lambda row: compute_qa_f1_score(row['response'], row['answer']), axis=1)
